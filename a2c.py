...
from a2c_agent import ActorCritic, Agent
...
class A2C:
    ...
    def __init__(self, n_agents, load=False, alpha=1e-3, gamma=0.99,
                 ...
                 state_type='only prices', djia_year=2014, repeat=0, entropy=1e-4):
        ...
        self.repeat = repeat
        self.start_date = datetime.datetime(2014, 5, 20)
        self.end_date = datetime.datetime(2024, 5, 20)
        self.download = False

        self.env = PortfolioEnv(self.start_date, self.end_date,self.download,action_scale=1000, state_type=state_type, djia_year=djia_year)
        if djia_year == 2014:
    ...
    def test(self, verbose=True):
        ...
        stats_a2c = timeseries.perf_stats(returns)

        buy_hold_returns = (buy_hold_history / buy_hold_history.shift(1) - 1).dropna()
        stats_buy_hold = timeseries.perf_stats(buy_hold_returns)

        combined_stats = pd.concat([stats_a2c, stats_buy_hold], axis=1)
        combined_stats.columns = ['DDPG', 'Buy&Hold']
